...
1. show me all the tran_date,tran_ammt and total tansaction ammount per tran_date
    ...
    select tran_date, tran_fact.tran_ammt, 
    sum(tran_fact.tran_ammt) over(partition by tran_date order by tran_date) as total_tran
...
2. show me all the tran_date,tran_ammt and total tansaction ammount per tran_date and rank of the transaction ammount desc within per tran_date
    ...
    select tran_date, tran_fact.tran_ammt, 
    sum(tran_fact.tran_ammt) over(partition by tran_date order by tran_date) as total_tran,
    rank() over(partition by tran_date order by tran_fact.tran_ammt desc)
    from cards_ingest.tran_fact
...
3. show me all the fields and total tansaction ammount per tran_date and only 2nd rank of the transaction ammount desc within per tran_date
 (Here you are using he question2 but filtering only for rank 2)
    ...
    from 
        (select *,
        sum(tran_fact.tran_ammt) over(partition by tran_date order by tran_date) as total_tran,
        rank() over(partition by tran_date order by tran_fact.tran_ammt desc) as tran_rank
        from cards_ingest.tran_fact) ranked
    where tran_rank = 2
...
1. Join tran_fact and cust_dim_details on cust_id and tran_dt between start_date and end_date
    ...
    select *
    from cards_ingest.tran_fact
    join cards_ingest.cust_dim_details on tran_fact.cust_id = cust_dim_details.cust_id and (tran_date >= start_date and tran_date <= end_date)
  ...
  cust_dim_details on cust_id and tran_dt between start_date and end_date
        ...
        from(
            ...
            from 
                ...
                rank() over(partition by tran_date order by tran_fact.tran_ammt desc) as tran_rank
                from cards_ingest.tran_fact) ranked
            where tran_rank = 2) table_1
        join cards_ingest.cust_dim_details on table_1.cust_id = cust_dim_details.cust_id and (tran_date >= start_date and tran_date <= end_date)
...
3. From question 2 : when stat_cd is not euqal to state_cd then data issues else good data as stae_cd_status
 ...
 to NULL Value to right table then it should be data issues]
    ...
    select tran_fact.cust_id, stat_cd,
    case tran_fact.cust_id = cust_dim_details.cust_id
    when tran_fact.stat_cd = cust_dim_details.state_cd then 'good data'
    else 'data issues'
    end as state_cd_status
    from cards_ingest.tran_fact
    join cards_ingest.cust_dim_details on tran_fact.cust_id = cust_dim_details.cust_id
